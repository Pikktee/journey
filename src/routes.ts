 ...
 * `path: '/anmelden'` wird dann `paths: { de: '/anmelden', en: '/sign-in' }`
 * samt `/en/`-Präfix, ohne dass eine einzige Link-Stelle im Code sich ändert —
 * die rufen alle `path('login')`. Ob die dann alten Pfade als Alias
 ...
 */
export const ROUTES: Readonly<Record<Page, Route>> = {
  start: { path: '/', file: 'index.html' },
  player: { path: '/erlebnis', file: 'erlebnis.html' },
  app: { path: '/app', file: 'studio.html' },
  login: { path: '/anmelden', file: 'studio.html' },
  register: { path: '/registrieren', file: 'studio.html' },
  gallery: { path: '/galerie', file: 'galerie.html' },
  profile: { path: '/profil', file: 'profil.html' },
  ...
  account: { path: '/konto', file: 'konto.html' },
  admin: { path: '/admin', file: 'admin.html' },
  ...
  feedback: { path: '/feedback', file: 'feedback.html' },
  imprint: { path: '/impressum', file: 'impressum.html' },
  privacy: { path: '/datenschutz', file: 'datenschutz.html' },
}

/**
 * Link auf eine Seite. `suffix` trägt Query und/oder Fragment mit —
 * `path('player', '?tour=srv:t_1')`, `path('login', '#verify=abc')`.
 */
export function path(page: Page, suffix = ''): string {
  return ROUTES[page].path + suffix
}
 ...
 * Wächter in test/routes.test.ts prüft das.
 */
export function tourPath(param: string): string {
  const id = param.startsWith('srv:') ? param.slice(4) : param
 ...
 * verwaisen.
 */
export function tourFromPath(pathname: string): string | null {
  const hit = /^\/tour\/([^/?#]+)/.exec(pathname)
  if (!hit?.[1]) return null
  const id = decodeURIComponent(hit[1])
  return id.startsWith('t_') ? `srv:${id}` : id
}
 ...
 */
export const PATH_TO_FILE: Readonly<Record<string, string>> = Object.fromEntries(
  Object.values(ROUTES).map((r) => [r.path, r.file]),
)
 ...
 */
export const ENTRIES: Readonly<Record<string, string>> = Object.fromEntries(
  [...new Set(Object.values(ROUTES).map((r) => r.file))].map((file) => [
    file.replace(/\.html$/, ''),
    file,
  ]),
)
