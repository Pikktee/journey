...
async function loadTile(tx: number, ty: number): Promise<Uint8ClampedArray> {
  ...
  if (!ctx) throw new Error('Kein 2D-Kontext für das Höhen-Sampling')
  ctx.drawImage(img, 0, 0)
  return ctx.getImageData(0, 0, SIZE, SIZE).data
}

// Höhe in Metern für jede Koordinate; wenige Tile-Fetches, Rest ist Pixel-Lookup
export async function sampleElevations(coords: LngLat[]): Promise<number[]> {
  ...
  return coords.map((c) => {
    const [x, y] = tileCoords(c[0], c[1])
    // Die Kachel steht in der Map — sie wurde eine Zeile weiter oben aus genau
    // diesen Koordinaten geladen; ebenso liegen die Pixel-Indizes im Puffer.
...
export function smoothValues(values: number[], win = 9): number[] {
