...
export function profilSichtbarSatz(oeffentlich: boolean, adresse: string): string {
  return oeffentlich
    ? `Dein Profil ist unter ${adresse} für alle sichtbar.`
 ...
 */
export function suchmaschinenSatz(an: boolean, profilOeffentlich: boolean): string {
  if (!profilOeffentlich) return 'Wirkt erst, wenn dein Profil öffentlich ist.'
  return an
