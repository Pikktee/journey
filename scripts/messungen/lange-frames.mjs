...
const browser = await chromium.launch({ channel: 'chromium', args: ['--mute-audio', '--autoplay-policy=no-user-gesture-required'] })
...
const r = await seite.evaluate(async (dauer) => {
  const loaf = []
  const po = new PerformanceObserver((l) => { for (const e of l.getEntries()) loaf.push(e.toJSON()) })
  try { po.observe({ type: 'long-animation-frame', buffered: true }) } catch { }
  const d = []
  ...
  await new Promise((fertig) => {
    const z = () => {
      ...
      d.push(n - last); last = n
      if (n < bis) requestAnimationFrame(z); else fertig()
    }
  ...
  return {
    frames: d.length, p50: q(0.5), p95: q(0.95), p99: q(0.99), max: d[d.length - 1],
    ueber33: d.filter((x) => x > 33).length, ueber50: d.filter((x) => x > 50).length, ueber100: d.filter((x) => x > 100).length,
    uhr: { ...window.__j.uhr },
    loaf: loaf.map((e) => ({
      ...
      scripts: (e.scripts || []).map((s) => ({
        d: Math.round(s.duration), inv: s.invoker, typ: s.invokerType,
...
console.log(`Frames ${r.frames} · p50 ${r.p50.toFixed(1)} ms · p95 ${r.p95.toFixed(1)} · p99 ${r.p99.toFixed(1)} · max ${r.max.toFixed(0)}`)
console.log(`>33ms: ${r.ueber33} (${(r.ueber33 / r.frames * 100).toFixed(2)} %) · >50ms: ${r.ueber50} · >100ms: ${r.ueber100}`)
console.log(`Uhr: verworfen ${r.uhr.verworfenS?.toFixed?.(2)} s in ${r.uhr.verworfenFrames} Frames · längstes Frame ${(r.uhr.laengstesFrameS * 1000).toFixed(0)} ms`)
...
for (const e of lang.slice(0, 20)) {
  console.log(`  ${e.dur} ms (blockierend ${e.block}, Style+Layout ${e.render})  ${e.scripts.map((s) => `${s.src}:${s.fn || s.inv} ${s.d}ms`).join(' | ') || '— kein Skript >3ms (Renderer/GPU/GC)'}`)
}
const nachFn = new Map()
for (const e of r.loaf) for (const s of e.scripts) {
  const k = `${s.src}:${s.fn || s.inv}`
  const v = nachFn.get(k) ?? { n: 0, sum: 0, max: 0 }
  v.n++; v.sum += s.d; v.max = Math.max(v.max, s.d); nachFn.set(k, v)
}
console.log('\nSkript-Zeit summiert (>=3 ms je Aufruf):')
for (const [k, v] of [...nachFn].sort((a, b) => b[1].sum - a[1].sum).slice(0, 15)) console.log(`  ${String(v.sum).padStart(6)} ms  n=${String(v.n).padStart(4)}  max ${String(v.max).padStart(4)} ms  ${k}`)
await browser.close()
