...
await seite.goto('http://maptale.localhost:5123/tour/t_cGuHmm3vMa4ggQ', { waitUntil: 'domcontentloaded' })
...
console.log(`  Frame-Zeit  p50 ${r.p50.toFixed(1)} ms · p95 ${r.p95.toFixed(1)} ms · max ${r.max.toFixed(0)} ms`)
console.log(`  Frames über dem 50-ms-Deckel: ${(r.anteilUeber * 100).toFixed(1)} %`)
console.log(`  Von ${r.echt.toFixed(1)} s Echtzeit zählt die Engine ${r.gezaehlt.toFixed(1)} s → verworfen: ${(r.echt - r.gezaehlt).toFixed(1)} s (${(((r.echt - r.gezaehlt) / r.echt) * 100).toFixed(1)} %)`)
await browser.close()
