...
//    (`STUETZ_MAX_M` in src/geo.ts) lief Stockholm auf 2,2 % seines Films mehr
...
//    Kameradistanz derselben Rampe wie das Tempo (`Filmspur.skalaBeiS`).
//
...
for (const tour of TOUREN) {
  ...
  const r = await seite.evaluate(() => {
    ...
    const fehler = []
    const schirm = []
    for (let f = 0; f <= a.gesamtS; f += 0.05) {
      if (t.film.haltBeiFilm(f)) continue
      const s = t.film.sBeiFilm(f)
      const vRoute = (t.film.sBeiFilm(f + H) - s) / H
