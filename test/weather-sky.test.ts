...
import { grading, skyAt, scrimFor, WEATHER_SKY, type SceneWeather } from '../src/weather-sky.js'
...
const TRAVEL_MODES: SceneWeather[] = ['off', 'clouds', 'fog', 'rain', 'snow', 'storm']
...
describe('himmelBei', () => {
  it('kennt jede Lage, die das Overlay setzen kann', () => {
    ...
    for (const m of WEATHER_MODES) expect(WEATHER_SKY[m as SceneWeather]).toBeDefined()
  })

  it('spannt die Deckung über die Stärke', () => {
    const leicht = skyAt('clouds', 0.4)
    const stark = skyAt('clouds', 1)
  ...
  it('lässt Niederschlag schon bei geringer Stärke bedeckt sein', () => {
    expect(skyAt('rain', 0.4).cover).toBeGreaterThan(skyAt('clouds', 0.4).cover)
    expect(skyAt('storm', 0.4).cover).toBeGreaterThan(skyAt('rain', 0.4).cover)
  })
  ...
  it('klemmt unter der kleinsten Stufe statt zu unterschreiten', () => {
    expect(skyAt('rain', 0).cover).toBeCloseTo(WEATHER_SKY.rain.c0, 3)
    expect(skyAt('rain', -1).cover).toBeCloseTo(WEATHER_SKY.rain.c0, 3)
  })

  it('gibt bei `off` überall null', () => {
    expect(skyAt('off', 1)).toEqual({ cover: 0, dark: 0, fog: 0 })
  })
})

describe('schleierFuer', () => {
  it('zeigt bei `off` nichts', () => {
    expect(scrimFor('off', 1)).toEqual({ wash: '', shadow: '', fogColor: 0, snow: 0 })
  })
  ...
  it('stellt JEDE Lage dar, auch die ohne Partikel', () => {
    for (const m of TRAVEL_MODES.filter((x) => x !== 'off')) {
      const s = scrimFor(m, 0.7)
      expect(s.wash, `${m} ohne Farbschleier`).not.toBe('')
      expect(s.shadow, `${m} ohne Abdunklung`).not.toBe('')
    }
  })

  it('wird mit der Stärke dichter', () => {
    const a = alpha(scrimFor('rain', 0.4).wash)
    const b = alpha(scrimFor('rain', 1).wash)
  ...
  // die dicht sein SOLL.
  it.each(['clouds', 'fog', 'rain', 'snow', 'storm'] as SceneWeather[])(
  ...
  it('gibt nur bei Schnee eine Schneedecke', () => {
    expect(scrimFor('snow', 1).snow).toBeGreaterThan(0)
    for (const m of TRAVEL_MODES.filter((x) => x !== 'snow')) {
      expect(scrimFor(m, 1).snow, `${m} sollte keinen Bodenschnee haben`).toBe(0)
    }
  })

  it('nennt Nebel nur, wo Nebel ist', () => {
    expect(scrimFor('fog', 1).fogColor).toBeGreaterThan(0.5)
    expect(scrimFor('clouds', 1).fogColor).toBe(0)
  })
})
...
describe('die Lagen sind voneinander zu unterscheiden', () => {
  ...
  const PAARE: Array<[SceneWeather, SceneWeather]> = []
  const LAGEN: SceneWeather[] = ['clouds', 'fog', 'rain', 'snow', 'storm']
  ...
  it('ordnet sich von hell nach dunkel wie die Lagen selbst', () => {
    const hell = (m: SceneWeather) => ueberGrau(m)[1] as number
...
function ueberGrau(m: SceneWeather, k = 1): number[] {
  ...
  return legen(legen(gegradet, zahl(s.wash)), zahl(s.shadow))
}
