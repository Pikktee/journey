...
describe('loeseFokusAuf', () => {
  ...
  it('gibt auch erkannten Modus-Kanten eine Identität', () => {
    ...
    const erstes = loeseFokusAuf(
      { kind: 'modus', bezugS: 600 },
      LEERES_OVERLAY,
      a,
      track,
      START,
    ...
    expect(zweites).toMatchObject({ mode: 'walk', from: iso(1800), naechsteAb: null })
  })
  ...
  it('hält eine Trim-Kante nicht für eine Modus-Grenze', () => {
    ...
    const ziel = loeseFokusAuf({ kind: 'modus', bezugS: 2400 }, e, abschnitte(e), track, START, [])
    expect(ziel?.from).toBeNull()
  })
  ...
  it('kennt die Grenze, die ein Band eröffnet UND die, die es schließt', () => {
    ...
    const mitte = loeseFokusAuf({ kind: 'modus', bezugS: 1800 }, e, abschnitte(e), track, START, [])
  ...
  it('Kamera: Grundband ohne Grenze, danach Preset samt Feinjustierung', () => {
    const e = mitKameraGrenze(LEERES_OVERLAY, iso(1800), 'near', 1.3)
    const grund = loeseFokusAuf({ kind: 'kamera', bezugS: 600 }, e, abschnitte(e), track, START, [])
    expect(grund).toMatchObject({ kind: 'kamera', from: null, naechsteAb: iso(1800) })
    expect(grund?.preset).toBeUndefined() // „Preset des Zuschauers"
    const nah = loeseFokusAuf({ kind: 'kamera', bezugS: 2400 }, e, abschnitte(e), track, START, [])
    expect(nah).toMatchObject({ preset: 'near', intensity: 1.3, from: iso(1800), naechsteAb: null })
  })

  it('Wetter: sobald eine Grenze gesetzt ist, gilt davor „klar" statt „automatisch"', () => {
    const ohne = loeseFokusAuf(
      { kind: 'wetter', bezugS: 600 },
      LEERES_OVERLAY,
      abschnitte(),
      track,
      START,
    ...
    const davor = loeseFokusAuf({ kind: 'wetter', bezugS: 600 }, e, abschnitte(e), track, START, [])
    expect(davor?.wetterMode).toBe('off') // das Overlay ersetzt das Auto-Wetter
    const regen = loeseFokusAuf(
      { kind: 'wetter', bezugS: 2400 },
      e,
      abschnitte(e),
      track,
      START,
    ...
    expect(regen).toMatchObject({ wetterMode: 'rain', intensity: 0.7, from: iso(1800) })
  })

  it('Moment ist ein Zeitpunkt: Anfang und Ende fallen zusammen', () => {
    const e = mitMoment(LEERES_OVERLAY, iso(900), 'orbit', 8)
    const ziel = loeseFokusAuf(
  ...
  it('Audio: Musik hat eine Spanne, ein Klang nur einen Zeitpunkt', () => {
    ...
    expect(
      loeseFokusAuf({ kind: 'audio', index: 0 }, e, abschnitte(e), track, START, []),
    ).toMatchObject({
      fromS: 600,
      toS: 1800,
      index: 0,
    })
    const klang = loeseFokusAuf({ kind: 'audio', index: 1 }, e, abschnitte(e), track, START, [])
    ...
    expect(
      loeseFokusAuf({ kind: 'audio', index: 0 }, offen, abschnitte(offen), track, START, [])?.toS,
    ).toBe(3600)
  })
...
describe('klemmeGrenze', () => {
  ...
  it('klemmt nur an vorhandenen Seiten', () => {
    ...
    expect(klemmeGrenze([{ from: iso(600) }], iso(600), START, -900)).toBe(-900)
  })
