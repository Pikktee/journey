// Drift-Wächter für das Design-System: DESIGN.md ist die Quelle, src/base.css
...
const BLAETTER = ['src/style.css', 'src/legal-text.css', 'src/page-elements.css', 'src/toolkit.css']
...
function token(name: string): string {
  ...
  expect(wert, `${name} fehlt in src/base.css`).toBeDefined()
  return wert ?? ''
}

/**
 * DESIGN.md-Name → CSS-Variable.
 *
 * Seit Welle 6 der Englisch-Migration heißt die Variable wie das Token: Aus
 * `primary` wird `--primary`. Übrig bleiben die Fälle, in denen DESIGN.md
 * ZWEI Namen für dieselbe Farbe führt (`amber` neben `primary`), und drei
 * Variablen, die anders heißen als ihr Token — sie stehen hier einzeln, damit
 * man sie sieht.
 */
const ALIASSE: Record<string, string> = {
  amber: '--primary',
  coral: '--secondary',
  'on-surface': '--text',
  surface: '--bg',
...
const cssName = (token: string): string => ALIASSE[token] ?? `--${token}`
...
describe('DESIGN.md ↔ src/base.css', () => {
  it('trägt jede Farbe aus dem YAML-Kopf mit demselben Wert', () => {
    ...
    for (const [name, wert] of Object.entries(farben)) {
      const hat = token(cssName(name))
      expect(
        gleich(hat, wert),
        `colors.${name}: DESIGN.md sagt ${wert}, base.css sagt ${hat}`,
      ).toBe(true)
    }
  })
  ...
  it('trägt jede Radius-Stufe mit demselben Wert', () => {
    ...
    for (const [name, wert] of Object.entries(stufen)) {
      const variable = name === 'card' ? '--radius-card' : `--radius-${name}`
      const hat = token(variable)
      expect(
        gleich(hat, wert),
        `rounded.${name}: DESIGN.md sagt ${wert}, base.css sagt ${hat}`,
      ).toBe(true)
    }
  })
  ...
  it('trägt Kopfleistenhöhe, Inhaltsbreite und Schatten aus DESIGN.md', () => {
    ...
    expect(gleich(token('--shadow'), tiefe.shadow ?? '')).toBe(true)
    expect(gleich(token('--focus-ring'), tiefe['focus-ring'] ?? '')).toBe(true)
  })
})
...
describe('Ein Namenssystem', () => {
  it('lässt keine HTML-Datei eigene Farb-Tokens definieren', () => {
    ...
    // --spur-x, --band-space …) — die gehören der Seite, nicht der Marke.
    for (const datei of SEITEN) {
      const eigene = [...lies(datei).matchAll(/^\s*(--[\w-]+):\s*(#[0-9a-fA-F]{3,8}|rgba?\()/gm)]
      expect(
        eigene.map((m) => m[1]),
        `${datei} definiert eigene Farb-Tokens — sie gehören nach src/base.css`,
      ).toEqual([])
    }
  })
  ...
  it('schreibt keinen Token-Wert irgendwo sonst roh hin', () => {
    ...
    // Regeln, obwohl daneben `--surface-1` und `--border` definiert waren.
    const werte = new Map(
  ...
  it('lässt keine Akzentfarbe roh in einer Regel stehen', () => {
    ...
    // `var(--primary)`, `var(--secondary)` oder `var(--on-cta)` zu kommen —
    // für Deckkraft über `color-mix(in srgb, var(--primary) 35%, transparent)`.
    ...
    for (const datei of [
      ...
      expect(
        funde(datei),
        `${datei} schreibt eine Akzentfarbe roh hin — var(--primary)/var(--secondary) oder color-mix nehmen`,
      ).toEqual([])
  ...
  it('zeigt den Fokus auf genau eine Art je Element', () => {
    ...
    // Rand vorhanden → Rand + `--focus-ring`, kein Rand → Outline mit 2px.
    //
    ...
    for (const datei of [...SEITEN, ...BLAETTER, 'public/404.html']) {
      ...
      for (const [block] of ohneKommentare.matchAll(/[^{}]*:focus[^{}]*\{[^}]*\}/g)) {
        ...
        expect(
          schatten.includes('var(--focus-ring)'),
          `${datei}: eigener Fokus-Schatten „${schatten}" — var(--focus-ring) nehmen`,
        ).toBe(true)
      }
    }
  })
  ...
  it('benutzt keine Variable, die es nicht gibt', () => {
    ...
    const quellen = [...SEITEN, ...BLAETTER, 'src/base.css', 'public/404.html']
...
describe('Die geteilten Blätter erreichen die Seiten', () => {
  it('bindet base.css in jeden Einstieg ein', () => {
    for (const datei of SEITEN) {
      const inhalt = lies(datei)
      const direkt = inhalt.includes('/src/base.css')
      // Impressum und Datenschutz ziehen sie über legal-text.css herein: Beide
      // kommen bewusst ohne JS aus und sollen bei EINEM Stylesheet bleiben.
      const ueberRechtstext =
        inhalt.includes('/src/legal-text.css') && lies('src/legal-text.css').includes('base.css')
  ...
  it('hält public/404.html von Hand auf denselben Werten', () => {
    ...
    for (const name of geprueft) {
      expect(
        ...
        `404.html: ${name} ist ${eigene[name]}, base.css sagt ${TOKENS[name]}`,
      ).toBe(true)
    }
    // Und sie darf keinen Verweis auf ein gehashtes Asset tragen.
    expect(seite).not.toContain('/src/base.css')
  })

  it('lädt die Blätter in der Reihenfolge, in der sie gelten müssen', () => {
    // Die Reihenfolge ist keine Kosmetik: `.am-item` aus page-elements.css
...
describe('basisZuerst() (vite.config.js)', () => {
  ...
  const bundle = {
    'assets/a.css': { source: ':root{--sheet-base:1}' },
    'assets/b.css': { source: ':root{--sheet-toolkit:1}' },
    'assets/c.css': { source: ':root{--sheet-page-elements:1}' },
