...
class TrackTest {
    ...
    fun `haelt die Punktzahl gedeckelt und behaelt die Form`() {
        var track = emptyList<TrackPoint>()
        repeat(1000) { i -> track = appendPoint(track, TrackPoint(8.0 + i * 1e-4, 50.0), hoechstzahl = 100) }
        assertTrue("bleibt unter der Grenze", track.size <= 100)
        // Anfang und Ende überleben das Ausdünnen — die Linie endet dort, wo
        // man gerade steht, und beginnt, wo man losgegangen ist.
        assertEquals(8.0, track.first().lng, 1e-9)
        assertEquals(8.0 + 999 * 1e-4, track.last().lng, 1e-9)
    }
    ...
    fun `verzerrt die Form nicht`() {
        ...
        val quadrat = listOf(
            TrackPoint(8.0, 50.0),
            TrackPoint(8.0 + laengenGrad, 50.0),
            TrackPoint(8.0 + laengenGrad, 50.001),
    ...
    fun `kommt mit einem einzigen Punkt und mit gar keinem zurecht`() {
        ...
        val einer = projectTrack(listOf(TrackPoint(8.0, 50.0)), 100f, 100f)
        assertEquals(1, einer.size)
        assertTrue(einer[0].x.isFinite() && einer[0].y.isFinite())
        assertTrue(projectTrack(emptyList(), 100f, 100f).isEmpty())
        assertTrue(projectTrack(listOf(TrackPoint(8.0, 50.0)), 0f, 0f).isEmpty())
    }
    ...
    fun `zieht eine Foto-Marke auf die gezeichnete Linie`() {
        ...
        val linie = listOf(ScreenPoint(0f, 0f), ScreenPoint(50f, 0f), ScreenPoint(100f, 0f))
        val gezogen = snapToLine(ScreenPoint(48f, 17f), linie)
        ...
        assertEquals(ScreenPoint(48f, 17f), snapToLine(ScreenPoint(48f, 17f), emptyList()))
    }

    @Test
    fun `fasst nah beieinander liegende Fotos zu einem Punkt zusammen`() {
        val marken = listOf(
            "m1" to ScreenPoint(10f, 10f),
            "m2" to ScreenPoint(14f, 12f), // derselbe Stopp — nur Pixel entfernt
            "m3" to ScreenPoint(80f, 80f),
        )
        val baelle = clusterPhotos(marken, mindestabstand = 12f)
