...
class FotoaufbereitungTest {
    ...
    fun `ein Kamerafoto schrumpft auf die Uploadkante`() {
        ...
        bereiteFotoFuerUpload(datei)
        val (b, h) = masse(datei)
        assertEquals(UPLOAD_KANTE, max(b, h))
    ...
    fun `die Aufnahme-Angaben ueberleben das Verkleinern`() {
        ...
        assertEquals(UPLOAD_KANTE.toString(), exif.getAttribute(ExifInterface.TAG_PIXEL_X_DIMENSION))
    }
