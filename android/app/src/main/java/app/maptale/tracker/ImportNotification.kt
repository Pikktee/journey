...
//
// Push (`MaptalePushService`) und der periodische Abruf (`TrackerPollWorker`)
...
import app.maptale.gallery.PhotoBackfillWorker
 ...
 * dafür ist der `PhotoBackfillWorker` zuständig, und zwar erst, wenn er fertig
 ...
 */
suspend fun notifyPendingImports(app: MaptaleApp) {
    if (!app.settings.currentAccount().loggedIn) return
    val offene = runCatching { app.apiClient.trackerPendingImports(quittieren = false) }.getOrElse { return }
    ...
    val backfillRunning = app.settings.currentAccount().autoPhotos && canReadGallery(app)
    ...
    runCatching { app.apiClient.trackerImportsSeen(erledigt) }
}
 ...
 */
suspend fun describeTours(
    app: MaptaleApp,
    importe: List<TrackerImport>,
    media: Int? = null,
): Pair<String, String>? {
    val allgemein = messageFor(importe) ?: return null
    ...
    val tour = runCatching { app.apiClient.tourList().firstOrNull { it.id == importe[0].tourId } }.getOrNull()
    val teile = buildList {
        if (anbieter.isNotBlank()) add(anbieter)
        tour?.km?.let { add(String.format(java.util.Locale.GERMANY, "%.1f km", it)) }
        media?.takeIf { it > 0 }?.let { add(if (it == 1) "1 Foto" else "$it Fotos") }
    }
    return (tour?.title?.ifBlank { null } ?: allgemein) to teile.joinToString(" · ")
}
 ...
 */
fun showProgress(context: Context, title: String, fertig: Int, gesamt: Int): Boolean {
 ...
 * danebenzustellen.
 */
fun showImportNotification(context: Context, title: String, unterzeile: String = ""): Boolean {
