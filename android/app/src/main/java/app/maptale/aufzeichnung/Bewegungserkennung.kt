...
// sondern in `Bewegungsdeutung` (pure, getestet).
//
...
object Bewegungserkennung {
    ...
    private val ARTEN = mapOf(
        DetectedActivity.ON_FOOT to Bewegungsart.ZU_FUSS,
        DetectedActivity.WALKING to Bewegungsart.ZU_FUSS,
        DetectedActivity.RUNNING to Bewegungsart.ZU_FUSS,
        DetectedActivity.ON_BICYCLE to Bewegungsart.RAD,
        DetectedActivity.IN_VEHICLE to Bewegungsart.FAHRZEUG,
    )

    fun darfErkennen(context: Context): Boolean =
    ...
    fun starte(context: Context) {
        if (!darfErkennen(context)) return
     ...
     */
    fun artAus(intent: Intent): Bewegungsart? {
        if (!ActivityTransitionResult.hasResult(intent)) return null
        val ereignisse = ActivityTransitionResult.extractResult(intent)?.transitionEvents ?: return null
        return ereignisse.lastOrNull { ARTEN.containsKey(it.activityType) }?.let { ARTEN[it.activityType] }
    }

    private fun intent(context: Context): PendingIntent = PendingIntent.getService(
        context,
        ANFRAGE_CODE,
        Intent(context, AufzeichnungsService::class.java).setAction(AufzeichnungsService.AKTION_BEWEGUNG),
