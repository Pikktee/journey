...
// `notifyPendingImports`, also in einem Push-Handler. Android gibt dem nur
...
import app.maptale.tracker.describeTours
import app.maptale.tracker.showProgress
import app.maptale.tracker.showImportNotification
...
class PhotoBackfillWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val tourId = inputData.getString(INPUT_TOUR_ID) ?: return Result.success()
        val importId = inputData.getString(INPUT_IMPORT_ID)
        if (!app.settings.currentAccount().loggedIn) return Result.success()
        ...
        // sie ist ja da.
        if (!app.settings.currentAccount().autoPhotos || !canReadGallery(app)) {
            return report(app, tourId, importId, 0)
        }
        ...
        val bilder = runCatching { findMatchingPhotos(app, tourId) }.getOrNull() ?: return spaeter(app, tourId, importId)
        // Nichts zu ergänzen: Die Tour ist vollständig, wie sie ist.
        if (bilder.isEmpty()) return report(app, tourId, importId, 0)
        ...
        // über dieselbe ID.
        val titelJetzt = describeTours(app, listOf(alsImport(tourId, importId)))?.first ?: ""
        showProgress(app, titelJetzt, 0, bilder.size)
        val geschafft = runCatching {
            uploadPhotos(app, tourId, bilder) { fertig, gesamt -> showProgress(app, titelJetzt, fertig, gesamt) }
        }.getOrDefault(0)
        ...
        return report(app, tourId, importId, geschafft)
    }
    ...
    private suspend fun report(app: MaptaleApp, tourId: String, importId: String?, media: Int): Result {
        ...
        if (showImportNotification(app, title, unterzeile) && importId != null) {
            runCatching { app.apiClient.trackerImportsSeen(listOf(importId)) }
        }
        return Result.success()
    }
    ...
    companion object {
        ...
        const val INPUT_IMPORT_ID = "importId"
