...
data class GalleryItem(
    ...
    val takenAtMs: Long,
    /** GPS aus dem Bild, wenn vorhanden — schlägt die Zeit (s. `anchor`). */
    val lat: Double? = null,
    val lng: Double? = null,
    ...
    val anchor: Pair<Double, Double>?
        get() = if (lat != null && lng != null) lng to lat else null
}
 ...
 */
fun isExtensionAllowed(fileName: String, isVideo: Boolean = false): Boolean {
    val extension = fileName.substringAfterLast('.', "").lowercase()
    return extension in (if (isVideo) ERLAUBTE_VIDEO_ENDUNGEN else ERLAUBTE_ENDUNGEN)
}

/** Ist das Bild eine echte Kameraaufnahme? */
fun isCameraImage(folder: String?): Boolean {
    val name = folder?.lowercase() ?: return false
 ...
 * lassen.
 */
fun searchWindow(startMs: Long, endMs: Long): LongRange =
    (startMs - TOLERANCE_MS)..(endMs + TOLERANCE_MS)
...
fun matchingImages(
    ...
    return bilder
        ...
        .filter { isCameraImage(it.folder) }
        .filter { isExtensionAllowed(it.fileName, it.isVideo) }
