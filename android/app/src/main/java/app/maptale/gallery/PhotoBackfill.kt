...
import app.maptale.upload.AddMediaItem
 ...
 * Vorschlag zustimmt.
 */
val READ_PERMISSION: String =
    if (Build.VERSION.SDK_INT >= 33) Manifest.permission.READ_MEDIA_IMAGES
    else Manifest.permission.READ_EXTERNAL_STORAGE

fun canReadGallery(context: Context): Boolean =
    ActivityCompat.checkSelfPermission(context, READ_PERMISSION) == PackageManager.PERMISSION_GRANTED
 ...
 * mit dem Zeitpunkt.
 */
suspend fun findMatchingPhotos(app: MaptaleApp, serverTourId: String): List<GalleryItem>? {
    ...
    if (!canReadGallery(app)) return emptyList()
    val fenster = runCatching { app.apiClient.tourTimeWindow(serverTourId) }.getOrNull() ?: return null
    val startMs = fromIso(fenster.first) ?: return null
    val endMs = fromIso(fenster.second) ?: return null
 ...
 *
 * `notifyPendingImports` läuft aus ZWEI Richtungen — der Push-Dienst weckt die
 * App, und der periodische `TrackerPollWorker` läuft ohnehin. Trifft eine
 ...
 */
internal fun fromIso(iso: String): Long? =
