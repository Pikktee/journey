...
fun zoomLevels(minRatio: Float, maxRatio: Float): List<ZoomLevel> {
    val stufen = mutableListOf<ZoomLevel>()
    if (minRatio < ULTRAWEIT_GRENZE) stufen += ZoomLevel(minRatio, formatZoom(minRatio))
    if (maxRatio >= 1f) stufen += ZoomLevel(1f, formatZoom(1f))
    if (maxRatio >= 2f) stufen += ZoomLevel(2f, formatZoom(2f))
    return stufen
}
 ...
 */
fun activeLevel(stufen: List<ZoomLevel>, ratio: Float): Int? {
