...
import app.maptale.tracker.notifyPendingImports
...
class MaptalePushService : FirebaseMessagingService() {
    ...
    override fun onRegistered(fid: String) {
        ...
        scope.launch {
            if (!app.settings.currentAccount().loggedIn) return@launch
            runCatching { app.apiClient.registerPushDevice(fid) }
    ...
    override fun onMessageReceived(message: RemoteMessage) {
        ...
        if (message.data["type"] != "import-finished") return
        ...
        scope.launch { notifyPendingImports(app) }
    }
}
