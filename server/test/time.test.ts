...
import { metersForFilmSeconds } from '../src/pipeline/film-tempo.js'
import {
  COLLAPSE_MIN_REMAINDER_M,
  RAMP_MIN_FILM_S,
  buildTimeSeries,
  distillTimeline,
  findPauses,
...
describe('findePausen', () => {
  it('findet eine stationäre Punktwolke (Mittagspause der App)', () => {
    const reihe = buildTimeSeries([marsch({ dauerS: 7200, pause: { abS: 1800, dauerS: 1500 } })])
  ...
  it('meldet keine Pause bei durchgehender Bewegung', () => {
    expect(findPauses(buildTimeSeries([marsch({ dauerS: 7200 })]))).toHaveLength(0)
  })
  ...
  it('behandelt eine Lücke MIT Ortswechsel nicht als Pause', () => {
    ...
    expect(findPauses(reihe)).toHaveLength(0)
  })
})

describe('raffePausen', () => {
  const mitPause = () =>
    buildTimeSeries([marsch({ dauerS: 7200, pause: { abS: 1800, dauerS: 1500 } })])
  ...
  it('lässt außerhalb des Rampenfensters die echte Zeit unangetastet', () => {
    ...
    const roh = compressPauses(reihe, [pause])
  ...
  it('verschmilzt überlappende Rampenfenster zweier dichter Pausen', () => {
    ...
    const reihe = buildTimeSeries([marsch({ dauerS: 3600, pause: { abS: 900, dauerS: 1200 } })])
    const pausen = findPauses(reihe)
    const roh = compressPauses(reihe, pausen)
    for (let i = 1; i < roh.length; i++) expect(roh[i]).toBeGreaterThanOrEqual(roh[i - 1] as number)
...
describe('kollabierePausen', () => {
  ...
  it('zieht das Naht-Duplikat an der Segmentgrenze mit — kein künstlicher Sprung', () => {
    ...
    const erg = collapsePauses(segmente)
  ...
  it('reicht Touren ohne Pause identisch durch', () => {
    const seg = marsch()
    const erg = collapsePauses([seg])
    expect(erg[0]).toBe(seg)
  })

  it('Timeline-Destillat bleibt nach dem Kollaps monoton (senkrechter f-Sprung)', () => {
    const erg = collapsePauses([marschMitDrift({ dauerS: 7200 })])
    const timeline = distillTimeline(buildTimeSeries(erg), '2026-07-04T06:00:00Z')
...
describe('destilliereTimeline', () => {
  ...
  it('destilliert konstantes Tempo auf zwei Stützstellen', () => {
    const timeline = distillTimeline(buildTimeSeries([marsch()]), START)
  ...
  it('rafft die Pause und behält Anfangs- wie Endzeit der Aufnahme', () => {
    const timeline = distillTimeline(
      buildTimeSeries([marsch({ dauerS: 7200, pause: { abS: 1800, dauerS: 1500 } })]),
      START,
    )
    if (!timeline) throw new Error('Timeline erwartet')
  ...
  it('gibt bei degenerierten Touren undefined zurück', () => {
    ...
    expect(distillTimeline(buildTimeSeries([marsch()]), 'kein-datum')).toBeUndefined()
    // keine Zeitspanne
    expect(
      distillTimeline(
        buildTimeSeries([
...
describe('positionZurZeit', () => {
  const reihe = buildTimeSeries([marsch({ dauerS: 3600 })])

  it('interpoliert linear zwischen den Punkten', () => {
    const mitte = positionAtTime(reihe, 1800)
    expect(mitte.f).toBeCloseTo(0.5, 2)
    expect(mitte.lat).toBeCloseTo(LAT, 6)
  })

  it('klemmt außerhalb der Zeitspanne auf die Enden', () => {
    expect(positionAtTime(reihe, -50).f).toBe(0)
    expect(positionAtTime(reihe, 99999).f).toBe(1)
  })
})
...
describe('zeitZurPosition (Umkehrung)', () => {
  ...
  it('führt zu genau der Zeit zurück, aus der der Anteil kam', () => {
    for (const t of [0, 137, 500, 862, 1000]) {
      const f = positionAtTime(reihe, t).f
      expect(timeAtPosition(reihe, f)).toBeCloseTo(t, 3)
    }
  })

  it('klemmt außerhalb von 0..1', () => {
    expect(timeAtPosition(reihe, -0.5)).toBe(0)
    expect(timeAtPosition(reihe, 4)).toBe(1000)
  })
  ...
  it('liefert an einer Pause den Moment des ANKOMMENS, nicht des Weiterfahrens', () => {
    ...
    const mitPause = buildTimeSeries([
      {
        mode: 'walk',
        pts: [
          [7.9, 46.5, 0, 0],
    ...
    const fPause = positionAtTime(mitPause, 3000).f
    expect(timeAtPosition(mitPause, fPause)).toBeCloseTo(600, 3)
    // Am Tourende greift dagegen die Klemme: dort gilt der letzte Zeitpunkt.
    expect(timeAtPosition(mitPause, 1)).toBe(4000)
  })
})
