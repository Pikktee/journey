...
import { placeMedia } from '../src/pipeline/placement.js'
import type { UploadMedium, UploadPoint } from '../src/schema/upload.js'

const START_MS = Date.parse('2026-07-04T08:00:00Z')
// Track mit Offsets 0 / 600 / 1800 s
const TRACK: UploadPoint[] = [
...
describe('platziereMedien', () => {
  it('verankert einen GPS-Anker nah am Track (gps)', () => {
    const [p] = placeMedia([medium({ anchor: [7.9105, 46.59] })], TRACK, START_MS)
  ...
  it('mappt ein Medium ohne Anker über die Aufnahmezeit (zeit)', () => {
    const [p] = placeMedia([medium({ takenAt: '2026-07-04T08:05:00Z' })], TRACK, START_MS)
    expect(p?.placement).toBe('time')
  ...
  it('lässt Medien außerhalb der Tour-Zeit unplatziert', () => {
    const vorher = placeMedia([medium({ takenAt: '2026-07-04T07:00:00Z' })], TRACK, START_MS)
    const nachher = placeMedia([medium({ takenAt: '2026-07-04T09:00:00Z' })], TRACK, START_MS)
  ...
  it('lässt bei zu kurzem Track alles unplatziert', () => {
    const [p] = placeMedia([medium({ anchor: [7.9105, 46.59] })], [TRACK[0]!], START_MS)
  ...
  it('platziert korrekt trotz unsortierter Track-Zeiten (Review-Fund)', () => {
    ...
    const [p] = placeMedia([medium({ takenAt: '2026-07-04T08:10:00Z' })], track, START_MS)
