...
import { DONE_RETENTION_DAYS, OPEN_RETENTION_DAYS } from '../src/feedback.js'
import { cleanContext } from '../src/routes/feedback.js'
...
describe('Rückmeldungen', () => {
  ...
  it('gibt bei kaputtem Kontext-JSON die Meldung trotzdem her', () => {
    ...
    expect(cleanContext('kein objekt')).toBeNull()
    expect(cleanContext(['auch', 'nicht'])).toBeNull()
    expect(cleanContext({ page: '   ' })).toBeNull()
  })
})
