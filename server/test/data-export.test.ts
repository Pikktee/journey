...
import { DataExportService, EXPIRY_HOURS, buildArchive } from '../src/data-export.js'
import { isCompressed, buildReadme, tourFolder } from '../src/data-export-content.js'
import { collectEntries, collectAccount, collectTours } from '../src/data-export-run.js'
import { formatSize } from '../src/routes/export.js'
...
async function createTour(u: TestUmgebung, no: string, title: string): Promise<string> {
...
describe('Auftragsverwaltung', () => {
  ...
  it('gibt ein abgelaufenes Archiv nicht mehr heraus', async () => {
    ...
    const dienst = new DataExportService(db, archive, () => jetzt)
    db.prepare(
  ...
  it('befreit ein Konto, dessen Lauf abgestürzt ist', async () => {
    ...
    const db = (await baueTestApp()).app.deps.db
    let jetzt = new Date('2026-08-06T10:00:00Z')
    const dienst = new DataExportService(db, new MemStorage(), () => jetzt)
    db.prepare(
...
describe('Inhalt', () => {
  it('benennt Tour-Ordner nach Nummer und Titel', () => {
    expect(tourFolder(3, 'Runde bei Frankfurt')).toBe('03-runde-bei-frankfurt')
    expect(tourFolder(12, 'Über den Grimselpass!')).toBe('12-ueber-den-grimselpass')
    // Ohne Titel bleibt die Nummer — sie ist je Konto eindeutig.
    expect(tourFolder(7, null)).toBe('07')
    expect(tourFolder(7, '???')).toBe('07')
  })
  ...
  it('erkennt, was schon komprimiert ist', () => {
    ...
    for (const n of ['a.jpg', 'b.JPEG', 'c.mp4', 'd.m4a', 'e.png'])
      expect(isCompressed(n), n).toBe(true)
    for (const n of ['tour.json', 'liesmich.txt', 'edits.json'])
      expect(isCompressed(n), n).toBe(false)
  })
...
describe('Größenangabe', () => {
  it('schreibt sie so, wie sie in der Mail stehen soll', () => {
    ...
    expect(formatSize(1024 * 1024 * 1024 * 1.4)).toBe('1,4 GB')
