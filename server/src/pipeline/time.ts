...
export function buildTimeSeries(segments: readonly UploadSegment[]): TimeSeries {
  ...
  for (const seg of segments) {
    for (const p of seg.pts) {
      ...
      if (vorher) dist += distanceM([vorher.lng, vorher.lat], [lng, lat])
 ...
 */
export function findPauses(reihe: TimeSeries): Pause[] {
  const { punkte } = reihe
  ...
  while (a < raster.length - 1) {
    const anker = punkte[raster[a] as number] as TimePoint
    let b = a
    while (b + 1 < raster.length) {
      const kandidat = punkte[raster[b + 1] as number] as TimePoint
      if (distanceM([anker.lng, anker.lat], [kandidat.lng, kandidat.lat]) > PAUSE_RADIUS_M) break
      b++
    }
    const ende = punkte[raster[b] as number] as TimePoint
 ...
 */
export function compressPauses(reihe: TimeSeries, pausen: readonly Pause[]): number[] {
  const { punkte } = reihe
  ...
  for (const pause of pausen) {
    const halbeM =
      metersForFilmSeconds(rampeFilmS(pause.dauerS), (punkte[pause.vonIdx] as TimePoint).mode) / 2
    const vonM = (punkte[pause.vonIdx] as TimePoint).dist - halbeM
    const bisM = (punkte[pause.bisIdx] as TimePoint).dist + halbeM
    let a = pause.vonIdx
    while (a > 0 && (punkte[a - 1] as TimePoint).dist >= vonM) a--
    let b = pause.bisIdx
    while (b < punkte.length - 1 && (punkte[b + 1] as TimePoint).dist <= bisM) b++
  ...
  for (const { a, b } of fenster) {
    const von = punkte[a] as TimePoint
    const bis = punkte[b] as TimePoint
 ...
 */
export function collapsePauses(segmente: readonly UploadSegment[]): UploadSegment[] {
  const reihe = buildTimeSeries(segmente)
  ...
  const punktZu = (i: number): TimePoint => reihe.punkte[i] as TimePoint
  const duplikat = (a: TimePoint, b: TimePoint): boolean =>
  ...
  for (const pause of pausen) {
    ...
    for (let i = von; i <= bis; i++) {
      ...
      const [lng = 0, lat = 0, ele = 0] = (segmente[seg] as UploadSegment).pts[pt] as UploadPoint
      sLng += lng
      sLat += lat
      sEle += ele
    }
  ...
  return buildTimeSeries(neu).gesamtM < COLLAPSE_MIN_REMAINDER_M ? [...segmente] : neu
}

/** Pseudo-Zeit der ganzen Tour je Punkt (echte Zeit + Zeitraffer an den Pausen). */
export function pseudoTimes(reihe: TimeSeries): number[] {
  return compressPauses(reihe, findPauses(reihe))
}
...
export function clockTimeAtFraction(
  ...
  const anteil = (i: number): number => (punkte[i] as TimePoint).dist / gesamtM
...
export function positionAtTime(
  ...
  const { punkte, gesamtM } = reihe
  const erster = punkte[0] as TimePoint
  const letzter = punkte[punkte.length - 1] as TimePoint
  const anteil = (p: TimePoint): number => (gesamtM > 0 ? p.dist / gesamtM : 0)
  ...
  while (lo < hi) {
    const mitte = (lo + hi) >> 1
    if ((punkte[mitte] as TimePoint).tSek < tSek) lo = mitte + 1
    else hi = mitte
  }
  const b = punkte[lo] as TimePoint
  const a = punkte[lo - 1] as TimePoint
 ...
 */
export function timeAtPosition(reihe: TimeSeries, f: number): number {
  const { punkte, gesamtM } = reihe
  const erster = punkte[0] as TimePoint | undefined
  const letzter = punkte[punkte.length - 1] as TimePoint | undefined
  ...
  while (lo < hi) {
    const mitte = (lo + hi) >> 1
    if ((punkte[mitte] as TimePoint).dist < ziel) lo = mitte + 1
    else hi = mitte
  }
  const b = punkte[lo] as TimePoint
  const a = punkte[lo - 1] as TimePoint
