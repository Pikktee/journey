...
export interface RailSource {
   ...
   */
  gleise(box: { sued: number; west: number; nord: number; ost: number }): Promise<RailPath[]>
}
...
export function boundingBox(segmente: readonly UploadSegment[]): {
  sued: number
  west: number
  nord: number
  ost: number
...
function mitBox(weg: RailPath): GleisMitBox {
  let sued = 90
  let west = 180
  let nord = -90
  let ost = -180
...
function aufGleis(p: UploadPoint, gleise: readonly GleisMitBox[]): boolean {
  const lng = p[0]
  const lat = p[1]
...
export function promoteRailSegments(
  ...
  return segmente.map((s) => {
    ...
    for (let i = 1; i < s.pts.length; i++)
      strecke += distanceM(s.pts[i - 1] as UploadPoint, s.pts[i] as UploadPoint)
    if (strecke < MIN_DISTANCE_M) return s
    const treffer = s.pts.filter((p) => aufGleis(p, mitBoxen)).length
    return treffer / s.pts.length >= ANTEIL ? { ...s, mode: 'tram' as TravelMode } : s
  })
}
...
export class FixedRails implements RailSource {
  ...
  constructor(private readonly antwort: RailPath[] = []) {}
