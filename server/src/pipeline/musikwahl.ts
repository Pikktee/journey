...
import { berechneStats } from './geo.js'
import type { WetterKeyframe } from './weather.js'
 ...
 */
export function nassAnteil(keyframes: readonly WetterKeyframe[]): number {
 ...
 */
export function waehleMusik(eingabe: MusikEingabe): string {
  ...
  if (nachts(beginn) && nachts(ende)) return AUTO_MUSIK.nachtfahrt

  // 2. Nässe über einem guten Drittel der Strecke.
  if (wetter && nassAnteil(wetter) >= NASS_AB) return AUTO_MUSIK.regentag

  // 3. Berge — Höhenmeter oder schiere Höhe.
  const stats = berechneStats(segmente)
  if (stats.gainM >= BERG_GEWINN_M || maxHoehe(segmente) >= BERG_HOEHE_M) return AUTO_MUSIK.bergpass

  // 4. Eine Fähre bedeutet Wasser — dafür ist die Küstenstraße da.
  if (segmente.some((s) => s.mode === 'ferry')) return AUTO_MUSIK.kuestenstrasse
