 ...
 */
const CARD_COLUMNS = `t.id, t.title, t.cover, t.cover_thumb, t.stats_json, t.created_at,
...
function asCard(z: GalleryRow) {
  ...
  const profilePublic = z.profile_visibility === 'public'
  const author2 = z.display_name
    ? {
        ...
        ...(profilePublic ? { id: z.autor_id, handle: z.autor_handle } : {}),
      }
    : null
  return {
    id: z.id,
...
export function registerGalleryRoutes(app: FastifyInstance): void {
  ...
  app.get<{ Querystring: { limit?: string; offset?: string } }>('/api/gallery', async (request) => {
    const limit = Math.min(Math.max(Number(request.query.limit) || PAGE_DEFAULT, 1), PAGE_MAX)
    ...
    const rows = db
      ...
      .all(limit + 1, offset) as GalleryRow[]

    return {
      tours: rows.slice(0, limit).map(asCard),
  ...
  app.get<{ Params: { id: string } }>('/api/users/:id/profile', async (request, reply) => {
    const who = request.params.id
    const userId = who.startsWith('u_') ? who : app.auth.userIdForHandle(who)
    const person = userId
      ? (db.prepare('SELECT id, created_at FROM users WHERE id = ?').get(userId) as
          { id: string; created_at: string } | undefined)
      : undefined
    const profile2 = person ? app.auth.profile(person.id) : null
    ...
    // Touren).
    const isOwner = !!person && request.user?.id === person.id
    if (!person || !profile2 || (profile2.visibility !== 'public' && !isOwner)) {
    ...
    return {
      ...
      instagram: profile2.instagram,
      avatarUrl: profile2.avatar
        ? `/api/users/${person.id}/avatar?v=${encodeURIComponent(profile2.avatar)}`
        : null,
      bannerUrl: bannerUrl(person.id, profile2.banner),
       ...
       * hineinsieht.
       */
      ownerOnly: profile2.visibility !== 'public',
      tours: rows.map(asCard),
    }
  })
}
