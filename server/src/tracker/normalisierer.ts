...
import { OhneRouteFehler, type RohPunkt, type RohTrack } from './vertrag.js'
 ...
 */
export const MAX_PUNKTE = 200_000
 ...
 */
export function zuGpx(track: RohTrack): string {
  if (track.format === 'gpx') {
    const text = new TextDecoder().decode(track.bytes ?? new Uint8Array())
    if (!text.includes('<trkpt')) throw new OhneRouteFehler()
    return text
  }
  if (track.format === 'punkte') return punkteZuGpx(track.punkte ?? [], track.titel)
