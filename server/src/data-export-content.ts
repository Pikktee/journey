 ...
 */
export function tourFolder(nummer: number, titel: string | null): string {
  const wort = (titel ?? '')
    .toLowerCase()
    .replace(/ä/g, 'ae')
    .replace(/ö/g, 'oe')
...
export const isCompressed = (name: string): boolean => SCHON_GEPACKT.test(name)
