...
export function istTitelbildVorschlag(wert: string): boolean {
 ...
 */
export function nacktesWeb(roh: string): string | null {
  const wert = roh
    .trim()
 ...
 */
export function nacktesInstagram(roh: string): string | null {
  const wert = roh
    .trim()
    .replace(/^[a-z][a-z0-9+.-]*:\/\//i, '')
